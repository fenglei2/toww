...
public class LearntDisMatrix {
	...
	public LearntDisMatrix(int rounds) {
		this.dissimilarityMatrix = DataModel.getInstance().getLogRankTestMatrix();
		this.rounds = rounds;
...
//		Util.printWholeMatrix(learntDissimilarity, size);
	...
	private void setValueForLearntDisMatrix(HashMap<Integer, Integer> clusterAssignmt) {
		int size = learntDissimilarity.keySet().size();
	...
	private int inSameCluster(int a, int b, HashMap<Integer, Integer> clusterAssignmt) {
	...
	public static Matrix mapToMatrix(int size,HashMap<Integer, HashMap<Integer, Double>> map) {
