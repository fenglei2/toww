...
public class DistanceCalulation {
	...
	public List<Cluster> mergeTwoClosestClusters(DistanceInfo distanceInfo,List<Cluster> partition) {
		...
		if (process)
			System.out.println("closcs.umbc pair: " + row  + "-" + col + " dis="+dis);
	...
	public void merge(int row, int column) {
		...
		for (int i = 0; i < rows.size(); i++) {
			if (i != row && i != column) {
				...
				if(process)
					System.out.println("merge "+row+","+column+" newdis to "+i+"="+distance);
	...
	private double clusterDistance(List<Integer> clusterCombine, List<Integer> cluster) {
