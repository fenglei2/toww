package com.cs.umbc.project.server.cccproject.PAM;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

import com.cs.umbc.project.server.cccproject.common.Distance;
...
public class MyKMedoidsDis {
	...
	public void cluster_swap() {
		...
		/*
		if (process) {
			System.out.println("build sse: " + sse);
			System.out.println(" --------   build result ------- ");
			printmap(cluster_result);
			System.out.println("\n  ---   start swap ---\n");
		}

		for (int i = 0; i < initMedoids.length; i++) {
			for (int h = 0; h < nonmedoids.size(); h++) {
				// swap medoid i and nonmedoid h
				swap(i, h);

				if (process) {
					System.out.print("round -> i: " + i + " h: " + h + "   ");
	...
	private void swap(int i, int h) {
		if (process) {
			System.out.println("<before swap>: " + medoids + " | " + nonmedoids);
		}
	...
	public HashMap<Integer, List<Pattern>> cluster() {
		...
		for (Pattern p : dataSet) {
			...
			for (int i = 0; i < medoids.size(); i++) {
				...
				double distance = Distance.calculateDissimlarity(p.getIndex(), medoids.get(i), disMatrix);
	...
	private void calculateSSE(HashMap<Integer, List<Pattern>> clusteredPatterns) {
		...
		for (int key : clusteredPatterns.keySet()) {
			...
			for (Pattern p : pats) {
				double distance = Distance.calculateDissimlarity(p.getIndex(), medoids.get(key), disMatrix);
				sse += distance;
			}
		}
	...
	private Pattern getFirstMedoid() {
		...
		for (Pattern candidatePattern : dataSet) {
			...
			for (Pattern mido : dataSet) {
				...
				double distance = Distance.calculateDissimlarity(candidatePattern, mido, disMatrix);
	...
	private Pattern maxDisSumToNonMedoids(List<Pattern> candidates, List<Pattern> medoidPatterns) {
		...
		for (Pattern candidatePattern : candidates) {
			...
			for (Pattern unselected : candidates) {
				...
				double dij = Distance.calculateDissimlarity(candidatePattern, unselected, disMatrix);
				double distToMedoid = selectClosestMedoid(medoidPatterns, unselected) - dij;
	...
	private double selectClosestMedoid(List<Pattern> medoidPatterns, Pattern candidate) {
		// Pattern closestMedoid=null;

		double minDis = Double.MAX_VALUE;
		for (Pattern mido : medoidPatterns) {
			double distance = Distance.calculateDissimlarity(candidate, mido, disMatrix);
